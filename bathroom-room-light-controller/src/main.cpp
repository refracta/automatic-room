...
const char *AP_MODE_SSID = "KITCHEN-LIGHT-CONTROLLER-1";
...
void setupServer() {
    ...
    server.on("/set", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (request->hasParam("status") && request->hasParam("type")) {
            ...
            int pin = type == "light2" ? LIGHT_PIN : FAN_PIN;
            bool requestStatus = status == "on";
            if (pin == FAN_PIN) {
                fanStatus = requestStatus;
            } else {
                lightStatus = requestStatus;
            }
            digitalWrite(pin, requestStatus ? LOW : HIGH);
            request->send_P(200, "application/json", successJson);
        } else {
            request->send_P(200, "application/json", R"rawliteral({"error":"parameter error"})rawliteral");
        }
